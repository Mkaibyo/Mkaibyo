...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        int end = str1.length() - overlap;
        String combonation = str1.substring(0, end);
        combonation += str2;
        return combonation;
    }
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        check.transferFrom(strSet);
        while (check.size() > 0) {
            String n = check.removeAny();
            strSet.add(n);
            assert false == check.contains(
                    n) : "Violation of CONTAINS_NO_SUBSTRING_PAIRS(strSet)";
        }
        Set<String> tmp = new Set1L<>();
        tmp.transferFrom(strSet);
        boolean toadd = true;
        while (tmp.size() > 0) {
            String t = tmp.removeAny();
            strSet.add(t);
            if (str.indexOf(t) >= 0) {
                strSet.remove(t);
            } else if (t.indexOf(str) >= 0) {
                toadd = false;
            }
        }
        if (toadd) {
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        while (!input.atEOS()) {
            ...
            addToSetAvoidingSubstrings(Set, str2);
        }
        return Set;
    }
